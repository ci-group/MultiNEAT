...
namespace NEAT
{
                   ...
                   unsigned int a_NumLayers = 1) // number of hidden layers. Each will have a_NumHidden nodes
    {
        ...
        if ((a_SeedType != 0) && (a_NumHidden > 0))
        {
            ...
            if (!a_FS_NEAT)
            {
                ...
                unsigned int last_src_id = 1;
                unsigned int prev_layer_size = a_NumInputs;
    ...
    void Genome::BuildHyperNEATPhenotype(NeuralNetwork &net, Substrate &subst)
    {
        ...
        if (subst.m_custom_connectivity.size() == 0)
        {
            ...
            for (long unsigned int i = net.NumInputs(); i < net.m_neurons.size(); i++)
            {
                // loop all neurons
                for (long unsigned int j = 0; j < net.m_neurons.size(); j++)
                {
    ...
    bool Genome::Mutate_AddNeuron(InnovationDatabase &a_Innovs, const Parameters &a_Parameters, RNG &a_RNG)
    {
        ...
        int t_tries = a_Parameters.NeuronTries;
        while (!t_link_found)
        {
            if (NumLinks() == 1)
            {
    ...
    bool Genome::Mutate_LinkWeights(const Parameters &a_Parameters, RNG &a_RNG)
    {
        ...
        unsigned int t_genometail = 0;
        if (NumLinks() > m_initial_num_links)
        {
            t_genometail = (unsigned int)(((double)(NumLinks())) * 0.8);
        }
    ...
    bool Genome::Mutate_NeuronActivations_A(const Parameters &a_Parameters, RNG &a_RNG)
    {
        ...
        for (unsigned int i = 0; i < NumNeurons(); i++)
        {
            ...
            if ((!a_Parameters.MutateOutputActivationFunction || m_NeuronGenes[i].Type() != OUTPUT) && (m_NeuronGenes[i].Type() != INPUT) && (m_NeuronGenes[i].Type() != BIAS))
            {
    ...
    bool Genome::Mutate_NeuronActivation_Type(const Parameters &a_Parameters, RNG &a_RNG)
    {
        ...
        if (a_Parameters.MutateOutputActivationFunction && m_NeuronGenes[t_choice].Type() == OUTPUT)
        {
            return false;
        }
